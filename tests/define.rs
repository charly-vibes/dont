...
fn define_with_well_formed_label_succeeds() {
    ...
    let output = dont()
        .args([
            ...
            "WB:P001",
            "--doc",
            "a Ricci tensor on a smooth manifold",
            "--label",
            "a Ricci tensor",
            "--json",
        ])
...
fn define_with_an_label_succeeds() {
    ...
    let output = dont()
        .args([
            ...
            "WB:P001",
            "--doc",
            "an amino acid found in dairy products",
            "--label",
...
fn define_without_label_emits_doc_shape_warning() {
    ...
    let output = dont()
        .args([
            ...
            "WB:P001",
            "--doc",
            "Ricci tensor. Extended explanation of what it is.",
            "--json",
        ])
        .env("DONT_DIR", dir.path())
        .assert()
    ...
    assert!(
        warnings
            .iter()
            .any(|w| w["rule_name"] == "term-doc-shape-indefinite"),
        "expected term-doc-shape-indefinite warning, got: {:?}",
        warnings
    );
}
...
fn define_with_where_clause_label_passes_sentence_check() {
    ...
    let output = dont()
        .args([
            ...
            "WB:P001",
            "--doc",
            "a pair (x, y) where x is an integer and y is a real number",
            "--label",
            "a pair (x, y) where x is an integer and y is a real number",
            "--json",
        ])
...
fn define_without_label_compound_doc_emits_no_doc_shape_warning() {
    ...
    let output = dont()
        .args([
            ...
            "WB:P001",
            "--doc",
            "a pair (x, y) where x and y are integers",
            "--json",
        ])
        .env("DONT_DIR", dir.path())
        .assert()
    ...
    let warnings = v["warnings"].as_array().unwrap();
    assert!(
        !warnings.iter().any(|w| w["rule_name"]
            .as_str()
            .unwrap_or("")
            .starts_with("term-doc-shape-")),
        "expected no doc-shape warnings for compound doc, got: {:?}",
        warnings
    );
}
