...
Notes:
  ...
  dont explain lockable --json")]
    ...
    #[command(after_help = "Examples:
  dont feedback bug --dry-run
  dont feedback feature --dry-run
  dont feedback bug --from-last-error")]
    Feedback {
        /// Kind of feedback: bug, feature, question.
        #[arg(value_name = "KIND")]
        kind: String,

        /// Include the last error context automatically.
        #[arg(long)]
...
fn emit_error_no_exit(err: ErrorResult, warnings: Vec<Warning>, code: i32) -> i32 {
    ...
    if human_mode() {
        ...
        // If no remediation was shown, offer feedback hint for non-zero exits
        if code != 0 && err.remediation.is_empty() {
...
fn main() {
    ...
    match command {
        ...
        // Feedback: report bugs and feature requests via GitHub issues.
        Command::Feedback {
            kind,
            from_last_error,
            dry_run,
        } => {
            use genesis::feedback::gh::{CreateIssueOptions, create_issue};
            use genesis::feedback::scratch::read_last_error;

            // Validate the kind
            let valid_kinds = ["bug", "feature", "question"];
            if !valid_kinds.contains(&kind.as_str()) {
                emit_error_and_exit(
                    ErrorResult::new(
                        "invalid-feedback-kind",
                        &format!(
                            "invalid feedback kind '{kind}'. Valid kinds: {}",
                            valid_kinds.join(", ")
                        ),
                        None,
                        None,
                        ...
                        vec![],
                        vec![RemediationEntry {
                            command: "dont feedback bug --dry-run".to_string(),
                            description: "Report a bug".to_string(),
                        }],
                    )
                    .unwrap(),
                    vec![],
            ...
            body.push_str("<!-- Auto-generated by dont feedback. Please edit as needed. -->\n\n");

            if from_last_error {
                // Include last error context
                let tool_name = "dont";
                if let Some(record) = read_last_error(tool_name) {
                    body.push_str("### Last Error Context\n\n");
                    ...
                    body.push_str("*(No recent error found in error scratch)*\n\n");
                }
            }

            body.push_str("### Description\n\n");
            body.push_str("<!-- Please describe the issue or feature request -->\n\n");
            body.push_str("### Steps to reproduce\n\n");
            body.push_str("<!-- If this is a bug report, please include steps to reproduce -->\n");

            // Determine the issue title
            let title = format!("[{}] Feedback from dont", kind);

            // Build the gh command for display
            let gh_cmd = format!(
                "gh issue create --repo {} --title {:?} --label {:?} --body-file -",
                repo_slug, title, kind
            );
            ...
            // Create the issue via genesis::feedback::gh
            match create_issue(&CreateIssueOptions {
                repo: repo_slug.clone(),
                title: title.clone(),
                body: body.clone(),
                labels: vec![kind.clone()],
                dry_run: false,
            }) {
                Ok(result) => {
                    let msg = match result {
                        genesis::feedback::gh::GhResult::Created { url, number } => {
                            format!("Issue #{} created: {}", number, url)
                        }
                        genesis::feedback::gh::GhResult::FallbackUrl(url) => {
                            format!("Issue not created. Open manually: {}", url)
                        }
                ...
                Err(err) => {
                    emit_error_and_exit(
                        ErrorResult::new(
                            ...
                            vec![],
                            vec![RemediationEntry {
                                command: gh_cmd.clone(),
                                description: "Create issue manually via GitHub CLI".to_string(),
                            }],
                        )
                        .unwrap(),
                        vec![],
