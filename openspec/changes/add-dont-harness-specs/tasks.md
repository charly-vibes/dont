...
- [x] 1.1 Write `dont-derived-queries` spec for `list` / `vocab` scope, universal `--as-of` filtering, and `--dependency-compromise` trace filter
...
- [x] 2.3 Include hardcoded tutorial, how-to entry points, and `sync-docs` strict marker refusal behaviour

## 3. Extract spawn protocol capability
- [x] 3.1 Write `dont-spawn-protocol` spec for `guess` / `assume` / `overlook` harness-mode behaviour
- [x] 3.2 Include explicit allowed terminal commands in payload, harness-mode detection order, and direct-mode override
- [x] 3.3 Include synchronous timeout expiry sweeps, `dont spawns` filters, and late-callback handling

## 4. Extract MCP transport capability
- [x] 4.1 Write `dont-mcp-interface` spec for optional `dont mcp` stdio server mode, exposing both read-only and mutating CLI verbs
...
## 5. Validate
- [ ] 5.1 Run `openspec validate add-dont-harness-specs --strict`
